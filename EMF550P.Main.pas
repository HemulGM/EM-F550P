...
uses
  ...
  Xml.XMLIntf, Vcl.StdCtrls, Xml.XMLDoc, System.Generics.Collections,
  HGM.Controls.VirtualTable, Vcl.Buttons, Vcl.ExtCtrls, Vcl.ComCtrls, ShellAPI,
  ShlObj, ActiveX, System.Win.ComObj, IniFiles, HGM.Controls.Labels, Vcl.ImgList,
  System.ImageList, Vcl.Grids, Xml.xmldom, MSXML;
  ...
  begin
    ...
    try
      try
       ...
       //Только XML-файлы
          if AnsiLowerCase(ExtractFileExt(FileName)) = '.xml' then
...
begin
  if FileOpenDialogInput.Execute then
    ProcessFiles(FileOpenDialogInput.Files);
end;
...
begin
 //Проверим реквизиты
  if EditINN.Text = '' then
    if Note('Укажите ИНН организации', neWarning, -1) then
      Exit;
  if EditKPP.Text = '' then
    if Note('Укажите КПП организации', neWarning, -1) then
      Exit;
  if EditOper.Text = '' then
    if Note('Укажите оператора (ФИО)', neWarning, -1) then
      Exit;
  if EditTel.Text = '' then
    if Note('Укажите контактный телефон оператора', neWarning, -1) then
      Exit;
 //
     ...
     //Формирование уведомления и сохранение
      ...
      //Путь сохранения
        case ComboBoxSave.ItemIndex of
          0:
            SDir := ExtractFileDir(FItem.FileName);
          1:
            SDir := EditSaveTo.Text;
        end;
...
//Поиск файла в списке
  ...
  begin
    ...
    for j := 0 to FFiles.Count - 1 do
      if AnsiLowerCase(FFiles[j].FileName) = AnsiLowerCase(FN) then
        Exit(True);
  end;
...
begin
  ...
  //Если файла из списка нет в нашем списке, то добавляем, в противном случае считаем его
    if not FileAlreadyExists(List[i]) then
      FFiles.Add(CreateInputFile(List[i]))
    else
    begin
      Inc(Found);
      LastFound := List[i];
 ...
 //Покажем пользователю результат добавления
  ...
  begin
    if Found = 1 then
      LastFound := 'Файл "' + LastFound + '" уже имеется в списке и не был добавлен.'
    else
      LastFound := 'В списке уже имются некоторые файлы (' + IntToStr(Found) + ') они не были добавлены';
...
begin
  if AdvSelectDirectory('', '', SDir, True, False, True) then
    EditSaveTo.Text := SDir;
end;

procedure TFormMain.TableExFilesDblClick(Sender: TObject);
begin
  if not IndexInList(TableExFiles.ItemIndex, FFiles.Count) then
    Exit;
...
begin
  Value := '';
  if not IndexInList(FRow, FFiles.Count) then
    Exit;
  case FCol of
    0:
      Value := IntToStr(FRow + 1);
    1:
      Value := ExtractFileName(FFiles[FRow].FileName);
    2:
      Value := InputFileStateStr(FFiles[FRow].State);
    3:
      Value := ExtractFileName(FFiles[FRow].Output);
    4:
      Value := 'Удалить';
  end;
end;
...
begin
  ...
  case Index of
    4:
      FFiles.Delete(TableExFiles.ItemIndex);
  end;
end;

procedure TFormMain.TableExNoteDblClick(Sender: TObject);
begin
  if not IndexInList(TableExNote.ItemIndex, FNotes.Count) then
    Exit;
  if not IndexInList(FNotes[TableExNote.ItemIndex].ItemIndex, FFiles.Count) then
    Exit;
...
begin
  if not IndexInList(ARow, FNotes.Count) then
    Exit;
  if ACol <> 0 then
    Exit;
  ...
  begin
    case FNotes[ARow].Event of
      neInfo:
        ImageList.Draw(TableExNote.Canvas, Rect.Left, Rect.Top, 1);
      neWarning:
        ImageList.Draw(TableExNote.Canvas, Rect.Left, Rect.Top, 2);
      neError:
        ImageList.Draw(TableExNote.Canvas, Rect.Left, Rect.Top, 3);
    end;
  end;
end;
...
begin
  Value := '';
  if not IndexInList(FRow, FNotes.Count) then
    Exit;
  case FCol of
    1:
      Value := EventNoteStr(FNotes[FRow].Event);
    2:
      Value := FNotes[FRow].Text;
  end;
end;
